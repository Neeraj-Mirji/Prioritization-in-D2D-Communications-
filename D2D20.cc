...
UePacketTrace (Ptr<OutputStreamWrapper> stream, const Address &localAddrs, std::string context, Ptr<const Packet> p, const Address &srcAddrs, const Address &dstAddrs)
{
 ...
 int shard1[10], shard2[10], shard3[10],shard4[10];
 int size1=0, size2=0, size3=0, size4=0;
  ...
  void printALL(d2d_user d2d[], cu_users cu[])
  {
        int i=0;
        std::cout<<std::endl<<std::endl<<" D2D Pair Details " <<std::endl;
        for(int i=0; i<60 ; i++) std::cout<<"*";
        for (i=0;i<10;i++)
        {
                std::cout<<"\n";        
                std::cout<<"Node Id : " <<d2d[i].Node_id<<std::endl;
                std::cout<<"Application Type : " <<d2d[i].application_type<<std::endl;
                std::cout<<"Cluster Id : " <<d2d[i].cluster_id<<std::endl;
        }
        
        std::cout<<std::endl<<std::endl<<" CU Details " <<std::endl;
        for(int i=0; i<60 ; i++) std::cout<<"*";
        for (i=0;i<4;i++)
        {
                std::cout<<"\n";        
                std::cout<<"CU : " << i+1 <<std::endl;
                std::cout<<"Application Type : " <<cu[i].application_type<<std::endl;
                std::cout<<"Cluster Id : " <<cu[i].cluster_id<<std::endl;
        }
...
{ 
    ...
    if(Simulator::Now().GetSeconds() >= 5.5 && Simulator::Now().GetSeconds() <= 5.50100)
    {
        ...
        for(int i=0; i< size2; i++) 
        {
                if(2*shard2[i] == ue && (i == 0 || i == 1 || i == 2 || i == 3))
                { 
                        //std::cout<<context<<std::endl;
                        std::cout<<" UE"<<ue<<" belonging to D2D"<<i+1<<"\n";
                        std::cout<<" Time : " <<Simulator::Now().GetSeconds()<<"\n";
...
// print SINR value of fourth shard

void PhySnirTrace4 (std::string context ,uint16_t cellId, uint16_t rnti, double rsrp, double sinr, uint8_t componentCarrierId)
{ 
...
int main (int argc, char *argv[])
{
  ...
  //The number of RBs allocated per UE for Sidelink ( Initialised down side )
  ...
  //Set the UEs power in dBm (initialised downside)
  ...
  //d2d devices
  d2d_user d2d[10];
  for(int i=0 ; i<10; i++) d2d[i].d2dNodes.Create (2);
    
  //CU devices
  cu_users cu[4];
  for(int i=0; i<4; i++) cu[i].Node.Create(1);
  ...
  positionAllocCu1->Add (Vector (-15.0, 5.0, 0.0));
  Ptr<ListPositionAllocator> positionAllocCu2 = CreateObject<ListPositionAllocator> ();
  positionAllocCu2->Add (Vector (20.0, 5.0, 0.0));
  Ptr<ListPositionAllocator> positionAllocCu3 = CreateObject<ListPositionAllocator> ();
  positionAllocCu3->Add (Vector (4.0, 3.0, 0.0));
  Ptr<ListPositionAllocator> positionAllocCu4 = CreateObject<ListPositionAllocator> ();
  positionAllocCu4->Add (Vector (-10.0, -7.0, 0.0));
  ...
  MobilityHelper mobilityCu4;
  mobilityCu4.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobilityCu4.SetPositionAllocator (positionAllocCu4);
  mobilityCu4.Install (cu[3].Node.Get (0));
  ...
  cu_positions[3] = positionAllocCu4->GetNext();

  
  Vector d2d_positions[20]; 
  ...
  d2d_positions[10] = positionAllocd2d11->GetNext();
  d2d_positions[11] = positionAllocd2d12->GetNext();
  d2d_positions[12] = positionAllocd2d13->GetNext();
  d2d_positions[13] = positionAllocd2d14->GetNext();
  d2d_positions[14] = positionAllocd2d15->GetNext();
  d2d_positions[15] = positionAllocd2d16->GetNext();
  d2d_positions[16] = positionAllocd2d17->GetNext();
  d2d_positions[17] = positionAllocd2d18->GetNext();
  d2d_positions[18] = positionAllocd2d19->GetNext();
  d2d_positions[19] = positionAllocd2d20->GetNext();

  d2d[0].application_type=2;
  d2d[1].application_type=1;
  d2d[2].application_type=2;
  d2d[3].application_type=2;
  d2d[4].application_type=2;
  d2d[5].application_type=2;
  d2d[6].application_type=2;
  d2d[7].application_type=1;
  d2d[8].application_type=1;
  d2d[9].application_type=2;
  for(int i=0 ; i<10 ; i++) d2d[i].store(d2d_positions[2*i], d2d_positions[2*i+1], i);
  for(int i=0 ; i<4 ; i++) cu[i].store(cu_positions[i]);
  ...
  NetDeviceContainer cuDevs4 = lteHelper->InstallUeDevice (cu[3].Node);
  randomStream += lteHelper->AssignStreams (cuDevs4, randomStream);
  ...
  //Control
  pfactory.SetControlPeriod ("sf60");
  ...
  if (!useIPv6)
    {
      ...
      Ipv4InterfaceContainer ueIpIface9;
      ueIpIface9 = epcHelper->AssignUeIpv4Address (NetDeviceContainer (cuDevs4));
      ...
      for (uint32_t u = 0; u < cu[1].Node.GetN (); ++u)
       ...
       for (uint32_t u = 0; u < cu[3].Node.GetN (); ++u)
        {
          Ptr<Node> ueNode = cu[3].Node.Get (u);
          // Set the default gateway for the UE
          Ptr<Ipv4StaticRouting> ueStaticRouting = ipv4RoutingHelper.GetStaticRouting (ueNode->GetObject<Ipv4> ());
          ueStaticRouting->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress (), 1);
        } 
  ...
  lteHelper->Attach (cuDevs4, enbDevs.Get (0));
  ...
  for(int i=0; i<2; i++) public_safety_centers[i].application = 2;
  social_media_centers = k_means(social_media, 2, ptr);
  for(int i=0; i<2; i++) social_media_centers[i].application = 1;
  
  //std::cout<<"After K means"<<std::endl;
  std::vector<point> centers;
  for(int i=0; i<2; i++) public_safety_centers.push_back(social_media_centers[i]);
  ...
  // assigning the nearest CU for a shard  
   ...
   for (int i = 0; i < 4; i++) 
   {
	...
	while(flag)
	{
	        temp = std::min_element(dists, dists + 4) - dists;
	        if(cu[temp].occupied) 
                        {dists[temp] = 1000000;}
	        else 
	        {
  ...
  for(int i=0; i<10; i++)
  {
     ...
     { 
      ...
      //Resource Allocation
      Config::SetDefault ("ns3::RrSlFfMacScheduler::SlGrantSize", UintegerValue (10));
      pfactory.SetControlPrbNum (24);
      pfactory.SetControlPrbStart (0);
      pfactory.SetControlPrbEnd (49);
  ...
  ApplicationContainer clientApps_shard4;
  ApplicationContainer serverApps_shard4;
  t=0;
  
  for(int i=0; i<10; i++)
  {
     if(d2d[i].application_type == 1 && d2d[i].cluster_id == 1)
     { 
      ApplicationContainer clientApps4 = sidelinkClient.Install (d2d[i].d2dNodes.Get (0));
      clientApps4.Start (Seconds(t+1.9));
      clientApps4.Stop (Seconds (11.0));
      clientApps_shard4.Add(clientApps4);
      shard4[t] = i;
      ...
      PacketSinkHelper sidelinkSink4 ("ns3::UdpSocketFactory", localAddress);
      serverApps4 = sidelinkSink4.Install (d2d[i].d2dNodes.Get (1));
      serverApps4.Start (Seconds (t+0.9));
      serverApps_shard4.Add(serverApps4);
  ...
  AsciiTraceHelper ascii1 , ascii2 , ascii3 , ascii4;
  Ptr<OutputStreamWrapper> stream1 = ascii1.CreateFileStream ("psc_shard1.tr");
  Ptr<OutputStreamWrapper> stream2 = ascii2.CreateFileStream ("psc_shard2.tr");
  Ptr<OutputStreamWrapper> stream3 = ascii3.CreateFileStream ("sm_shard1.tr");
  Ptr<OutputStreamWrapper> stream4 = ascii4.CreateFileStream ("sm_shard2.tr");

  //Trace file table header
  *stream1->GetStream () << "time(sec)\ttx/rx\tNodeID\tIMSI\tPktSize(bytes)\tIP[src]\tIP[dst]" << std::endl;
  *stream2->GetStream () << "time(sec)\ttx/rx\tNodeID\tIMSI\tPktSize(bytes)\tIP[src]\tIP[dst]" << std::endl;
  *stream3->GetStream () << "time(sec)\ttx/rx\tNodeID\tIMSI\tPktSize(bytes)\tIP[src]\tIP[dst]" << std::endl;
  *stream4->GetStream () << "time(sec)\ttx/rx\tNodeID\tIMSI\tPktSize(bytes)\tIP[src]\tIP[dst]" << std::endl;
      ...
      for (uint16_t ac = 0; ac < clientApps_shard1.GetN (); ac++)
        {
          ...
          clientApps_shard1.Get (ac)->TraceConnect ("TxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, stream1, localAddrs));
          oss.str ("");
        }  
      ...
      for (uint16_t ac = 0; ac < clientApps_shard2.GetN (); ac++)
        {
          ...
          clientApps_shard2.Get (ac)->TraceConnect ("TxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, stream2, localAddrs));
          oss.str ("");
        }
      ...
      for (uint16_t ac = 0; ac < clientApps_shard3.GetN (); ac++)
        {
          ...
          clientApps_shard3.Get (ac)->TraceConnect ("TxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, stream3, localAddrs));
          oss.str ("");
        } 
        

      for (uint16_t ac = 0; ac < clientApps_shard4.GetN (); ac++)
        {
          Ipv4Address localAddrs =  clientApps_shard4.Get (ac)->GetNode ()->GetObject<Ipv4L3Protocol> ()->GetAddress (1,0).GetLocal ();
          d2d[shard4[ac]].address1 = localAddrs;
          oss << "tx\t" << d2d[shard4[ac]].d2dNodes.Get (0)->GetId () << "\t" << d2d[shard4[ac]].d2dNodes.Get (0)->GetDevice (0)->GetObject<LteUeNetDevice> ()->GetImsi ();
          clientApps_shard4.Get (ac)->TraceConnect ("TxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, stream4, localAddrs));
          oss.str ("");
        } 
      ...
      for (uint16_t ac = 0; ac < serverApps_shard1.GetN (); ac++)
        {
          ...
          serverApps_shard1.Get (ac)->TraceConnect ("RxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, stream1, localAddrs));
          oss.str ("");
        }
      ...
      for (uint16_t ac = 0; ac < serverApps_shard2.GetN (); ac++)
        {
          ...
          serverApps_shard2.Get (ac)->TraceConnect ("RxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, stream2, localAddrs));
          oss.str ("");
        }
      ...
      for (uint16_t ac = 0; ac < serverApps_shard3.GetN (); ac++)
        {
          ...
          serverApps_shard3.Get (ac)->TraceConnect ("RxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, stream3, localAddrs));
          oss.str ("");
        }

      for (uint16_t ac = 0; ac < serverApps_shard4.GetN (); ac++)
        {
          Ipv4Address localAddrs =  serverApps_shard4.Get (ac)->GetNode ()->GetObject<Ipv4L3Protocol> ()->GetAddress (1,0).GetLocal ();
          d2d[shard4[ac]].address2 = localAddrs;
          oss << "rx\t" << d2d[shard4[ac]].d2dNodes.Get (1)->GetId () << "\t" << d2d[shard4[ac]].d2dNodes.Get (1)->GetDevice (0)->GetObject<LteUeNetDevice> ()->GetImsi ();
          serverApps_shard4.Get (ac)->TraceConnect ("RxWithAddresses", oss.str (), MakeBoundCallback (&UePacketTrace, stream4, localAddrs));
          oss.str ("");
        }
  ...
  std::cout<<"\n";
  std::cout<<"\n";
  std::cout<<"    SINR values of Shard 2 public saftey \n";
  std::cout<<"\n";
  ...
  //std::cout<<"\n";
  //std::cout<<"    SINR values of Shard 4  \n";
  //std::cout<<"\n";
  
  //Config::Connect ("/NodeList/*/DeviceList/0/$ns3::LteUeNetDevice/ComponentCarrierMapUe/*/LteUePhy/ReportCurrentCellRsrpSinr", MakeCallback (&PhySnirTrace4));


  Simulator::Run ();
  
  
 // Calculation of Throughput , end to end delay , Avg Throughput , Sent Packets , Received Packets , Lost Packets through Flow Monitor
